...
class FinancieraSiisaCda(models.Model):
	_name = 'financiera.siisa.cda'

	_order = 'orden asc'
	name = fields.Char('Nombre')
	activo = fields.Boolean('Activo')
	orden = fields.Integer('Orden de ejecucion')
	otorgar_cpm_base = fields.Float('SIISA - CPM Base', digits=(16,2))
	otorgar_cpm_maximo = fields.Float('SIISA - CPM Maximo', digits=(16,2))
	otorgar_partner_tipo_id = fields.Many2one('financiera.partner.tipo', 'Tipo de cliente')
	regla_ids = fields.One2many('financiera.siisa.cda.regla', 'siisa_cda_id', 'Reglas')
	...
	def ejecutar(self, informe_id):
		...
		cda_resultado_id = self.env['financiera.siisa.cda.resultado'].create({
			'name': self.name,
		...
		for regla_id in self.regla_ids:
			...
			regla_resultado_id.siisa_cda_id = None
			regla_resultado_id.siisa_cda_resultado_id = cda_resultado_id.id
...
class FinancieraSiisaCdaResultado(models.Model):
	_name = 'financiera.siisa.cda.resultado'

	_order = 'id desc'
	name = fields.Char('Nombre')
	informe_id = fields.Many2one('financiera.siisa.informe', 'Informe')
	regla_ids = fields.One2many('financiera.siisa.cda.regla', 'siisa_cda_resultado_id', 'Reglas')
	otorgar_cpm = fields.Float('SIISA - CPM', digits=(16,2))
	otorgar_cpm_maximo = fields.Float('SIISA - CPM Maximo', digits=(16,2))
	otorgar_partner_tipo_id = fields.Many2one('financiera.partner.tipo', 'Tipo de cliente')
	resultado = fields.Selection([('rechazado', 'Rechazado'), ('aprobado', 'Aprobado')], 'Resultado')

class FinancieraSiisaCdaRegla(models.Model):
	_name = 'financiera.siisa.cda.regla'

	siisa_cda_id = fields.Many2one('financiera.siisa.cda', 'CDA Evaluacion')
	siisa_cda_resultado_id = fields.Many2one('financiera.siisa.cda.resultado', 'CDA Resultado')
	...
	def ejecutar(self, informe_id):
		self.ensure_one()
		variable_obj = self.pool.get('financiera.siisa.informe.variable')
		...
		if len(variable_ids) > 0:
			...
			if self.resultado == 'aprobado':
				self.siisa_cda_resultado_id.otorgar_cpm *= self.cpm_multiplicar
